...
[[ $# -lt 3 ]] && echo "Usage: $0 <VxLAN> <udp-port> <remote.pub.ip6> [remote.lan.ip4/net] " && exit 255
...
#REMOTE_IP6=$3  #$(echo $SSH_CLIENT | cut -d' ' -f1)
#REMOTE_IP6=${2%:*}
#REMOTE_PORT=${2##*:}
REMOTE_PORT=$2
REMOTE_IP6=$3
...
for net in $*;do
  ip route add $net via $REMOTE_INT metric 200 dev $TUN_DEV
done
